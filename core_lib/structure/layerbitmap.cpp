...
Status LayerBitmap::saveKeyFrame(KeyFrame* pKeyFrame, QString path)
{
    ...
    if (pBitmapImage->isModified() == false)
        return Status::SAFE;

    if (pBitmapImage->image()->isNull())
        return Status::SAFE;

    QString theFileName = fileName(pKeyFrame);
    ...
    if (!pBitmapImage->image()->save(strFilePath))
    {
        ...
        debugInfo << "LayerBitmap::saveKeyFrame"
            << QString("  KeyFrame.pos() = %1").arg(pKeyFrame->pos())
    ...
    pBitmapImage->setModified(false);
    return Status::OK;
}
